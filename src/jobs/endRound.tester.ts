...
export const endRoundTest = async (roundId: string, nsp: Namespace, presence: Presence): Promise<void> => {
  try {
    // ---- Load state ------//
    const [round, settings, companyWallet] = await Promise.all([
      Round.findById(roundId),
    ...
    //=====================================
    // Status: PUBLIC,   desc: Close Round emit
    //=====================================
    ...
    console.log("=== ROUND DEBUG START ===");
    console.log(`Round #${round.roundNumber} (${round._id})`);
    console.log(`Total bets: ${totalPool}`);
    ...
    if (!winnerBox) {
      ...
      //=====================================
      // Status: PUBLIC,   desc: round completed
      //=====================================
      nsp.emit(EMIT.ROUND_ENDED, {
        _id: round._id,
        roundNumber: round.roundNumber,
      ...
      //=====================================
      // Status: PUBLIC,   desc: Reset round, prepare for next 
      //=====================================
      nsp.emit(EMIT.ROUND_RESET, {
        _id: "",
        roundNumber: round.roundNumber,
    ...
    // Only continue if users are still online now
    if (presence.hasActiveUsers()) {
      return startNewRound(nsp, presence);
    }

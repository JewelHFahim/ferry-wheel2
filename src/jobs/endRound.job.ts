...
export const endRound = async (roundId: string, nsp: Namespace, presence: Presence): Promise<void> => {
  try {
    // ---- Load state ------//
    const [round, settings, companyWallet] = await Promise.all([
      Round.findById(roundId),
    ...
    // per-group aggregates (only member boxes + include direct bets on rep)
    let pizzaTotal = 0, pizzaCount = 0;
    let saladTotal = 0, saladCount = 0;
    for (const b of bets) {
      if (pizzaMembers.has(b.box)) { pizzaTotal += b.amount; pizzaCount += 1; }
    ...
    const companyCut = Math.floor(totalPool * commissionRate);
    round.companyCut = companyCut;
    const distributableAmount = totalPool - companyCut;
    const availableFunds = distributableAmount + companyWallet.reserveWallet;
    ...
    if (!winnerBox) {
      ...
      console.log("=== ROUND DEBUG END (no eligible winner) ===");
      // schedule next round only if users still online
      if (presence?.hasActiveUsers?.() === true) {
        setTimeout(() => startNewRound(nsp, presence), prepareDuration);
      } else {
        console.log("[engine] stop: no active users after round completion");
      }
      return;
    }
    ...
    // announce winner
    round.roundStatus = ROUND_STATUS.REVEALED;
    await round.save();
    nsp.emit(EMIT.WINNER_REVEALED, {
      _id: round._id,
      roundNumber: round.roundNumber,
    ...
    if (fresh) {
      ...
      // Treasury: company cut + leftover distributable
      const remainingDistributable = distributableAmount - reserveUsed - (totalPayout - reserveUsed);
      // remainingDistributable simplifies to: distributableAmount - totalPayout
      const leftover = distributableAmount - totalPayout;

      await addRoundFunds(companyCut, Math.max(0, leftover));
      round.reserveWallet = Math.max(0, leftover);
      await round.save();
      await logTransaction("companyCut", companyCut, "Company cut from pool");
      ...
      try {
        ...
        await logRoundEvent({
          ...
          rewardAmount: totalPayout,
          platformRevenue: companyCut,
          platformReserve: Math.max(0, leftover),
          gameVictoryResult: normalRequiredRows.map(r => ({ box: r.box, boxTotal: r.boxTotal })),
          winnerBox,
          date: new Date(),
        });
    ...
    // schedule next round ONLY if users still online
    if (presence?.hasActiveUsers?.() === true) {
      setTimeout(() => startNewRound(nsp, presence), 0);
    } else {
      console.log("[engine] stop: no active users after round completion");
    }
  } catch (err) {

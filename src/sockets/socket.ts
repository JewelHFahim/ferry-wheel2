...
export const initSocket = (server: http.Server) => {
  ...
  async function maybeStartEngine() {
    if (!presence.hasActiveUsers()) {
      console.log("[loop] not starting: no active users");
      return;
    }
    if (roundLoopRunning) return;
    roundLoopRunning = true;
    try {
      await startNewRound(game, presence);
    } finally {
  ...
  game.on("connection", (socket) => {
    ...
    // ping
    socket.on(EMIT.PING_SERVER, (_data, callback) => { callback({ ok: true }) });
    ...
    socket.on(EMIT.GET_CURRENT_ROUND, async (_payload, ack) => {
      try {
        ...
        let userPerBoxTotal: any[] = [];
        if (userId) { userPerBoxTotal = await getUserPerboxTotal(userId, snap._id) };

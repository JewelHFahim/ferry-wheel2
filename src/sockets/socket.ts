...
// // Emidiate response for current perbox total
...
import { createPresence, Presence } from "./presence";
import { startNewRound } from "../jobs/startNewRound.job";

export const initSocket = (server: http.Server) => {
  const io = new Server(server, { cors: { origin: origins, methods: ["GET","POST"], credentials: false } });
  const game = io.of("/game");

  socketAuthMiddleware(game, { strict: true, joinRooms: true });

  // presence + single-flight guard
  const presence: Presence = createPresence(game);
  ...
  async function maybeStartEngine() {
    ...
    try {
      await startNewRound(game, presence); // this function will self-schedule next rounds only if presence is true
    } finally {
      // if startNewRound returns (e.g., because presence went false and it didn’t schedule a next round),
      // allow a future connect to start it again.
      roundLoopRunning = false;
      // If users are still here, immediately try again (covers races)
      if (presence.hasActiveUsers()) {
        maybeStartEngine();
      }
    }
  }
  ...
  game.on("connection", (socket) => {
    ...
    presence.wireSocket(socket);
    // try to kick engine whenever a user connects
    void maybeStartEngine();

    socket.on(EMIT.PING_SERVER, (_d, ack) => ack?.({ ok: true }));

    socket.on(EMIT.GET_CURRENT_ROUND, async (_payload, ack) => {
      try {
        const snap = await MetService.getCurrentRoundSnapshot();
        if (!snap) return ack?.({ success:false, message:"No active round", round:null, userPerBoxTotal:[] });

        let userPerBoxTotal: any[] = [];
        if (user?._id) userPerBoxTotal = await getUserPerboxTotal(user._id, snap._id);

        let phaseEndTime = snap.endTime;
        if (snap.roundStatus === "revealing") phaseEndTime = snap.revealTime;
        else if (snap.roundStatus === "prepare") phaseEndTime = snap.prepareTime;

        ack?.({ success:true, round:snap, userPerBoxTotal, phaseEndTime });
      } catch (e:any) {
        ack?.({ success:false, message:e?.message || "server error" });
      }
    });

    socket.on("join", async (_payload, ack) => {
      try {
        handleJoinRoom(socket);
        if (user?._id) {
          const meta = await MetService.getMeta();
          if (meta?.currentRoundId) {
            const upt = await getUserPerboxTotal(user._id, String(meta.currentRoundId));
            game.to(`user:${user._id}`).emit(EMIT.USER_PERBOX_TOTAL, { roundId:String(meta.currentRoundId), userId:user._id, userPerBoxTotal:upt });
          }
        }
        ack?.({ success:true });
      } catch (e:any) {
        ack?.({ success:false, message:e?.message || "join failed" });
      }
    });

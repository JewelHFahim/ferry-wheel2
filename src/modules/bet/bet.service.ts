...
export const placeBet = async ({ userId, roundId, box, amount, nsp }: PlaceBetArgs) => {
  // Fetch required data concurrently
  const [round, settings, user] = await Promise.all([
    Round.findById(roundId).lean(),             // lean for quick read
    SettingsService.getSettings(),
    UserService.getById(userId),
  ]);
  ...
  // Guard: prevent direct bets on representatives (optional – remove if you want to allow)
  if (box === "Pizza" || box === "Salad") {
    throw new BetError(gameCodes.INVALID_BOX, "Direct bets on Pizza/Salad are not allowed");
  }

  // Determine if the chosen box belongs to Pizza/Salad group using *round.boxStats* truth
  ...
  // Deduct balance first (simple approach; wrap in tx later when you enable replica set)
  await UserService.updateBalance(userId, -amount);
  ...
  // Atomically update:
  //  - the exact box
  //  - and (if applicable) the representative group row (Pizza/Salad)
...
if (groupRep && groupRep !== box) {
  ...
  // Get the updated document so we can emit fresh numbers
const updatedRound = await Round.findOneAndUpdate(
  { _id: roundId, roundStatus: ROUND_STATUS.BETTING }, // critical gate
  { $inc: incSpec },
  {
    new: true,
    arrayFilters,
...
nsp.emit(EMIT.ROUND_TOTAL_BET, {
  ...
  // Emit updated round data with live group totals reflected
nsp.emit(EMIT.ROUND_UPDATED, {
  _id: roundId,
  roundNumber: updatedRound.roundNumber,
